...
function toggleDarkMode() {
    ...
    console.log('[Dark Mode Toggle] darkMode is now:', darkMode);
    ...
    try { 
        ...
        console.log('[Dark Mode Toggle] Saved to localStorage:', darkMode ? '1' : '0');
    } catch (e) {
    ...
    if (typeof updateDashboardThemeBtn === 'function') {
        ...
        console.log('[Dark Mode Toggle] Dashboard button updated');
    } else {
        console.warn('[Dark Mode Toggle] updateDashboardThemeBtn not found!');
    }
}
...
function updateDashboardThemeBtn() {
    console.log('[Dashboard Theme] Called with darkMode:', darkMode);
    
    if (!translations[currentLang] || !translations[currentLang].dashboard) {
        console.warn('[Dashboard Theme] Translations not available!');
        return;
    }
    
    const themeBtn = document.querySelector('.dashboard-btn[data-action="theme"]');
    console.log('[Dashboard Theme] Button found:', !!themeBtn);
    
    if (themeBtn) {
        const icon = themeBtn.querySelector('i');
        const span = themeBtn.querySelector('span');
        
        console.log('[Dashboard Theme] Icon:', !!icon, 'Span:', !!span);
        
        if (darkMode) {
            icon.className = 'fas fa-sun';
            span.textContent = translations[currentLang].dashboard.lightMode || 'Tryb jasny';
            console.log('[Dashboard Theme] Set to LIGHT MODE icon (sun)');
        } else {
            icon.className = 'fas fa-moon';  
            span.textContent = translations[currentLang].dashboard.darkMode || 'Tryb ciemny';
            console.log('[Dashboard Theme] Set to DARK MODE icon (moon)');
        }
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Initialize theme (combines loading saved preference and setting up UI)
    initializeTheme();
    ...
    }, 200); // Zwiększamy opóźnienie do 200ms
