...
function toggleSizeMenu(matrixId) {
    ...
    document.querySelectorAll('.size-menu').forEach(m => {
        if (m !== menu && m.classList.contains('open')) {
            ...
            setTimeout(() => {
                ...
                // Usuń backdrop dla innych menu
                const backdrop = document.querySelector('.size-menu-backdrop');
    ...
    if (menu.classList.contains('open')) {
        ...
        if (isMobile) {
            // Dla mobile: pełnoekranowe menu z backdrop
            
            // Utwórz backdrop
            let backdrop = document.querySelector('.size-menu-backdrop');
            if (!backdrop) {
                backdrop = document.createElement('div');
                backdrop.className = 'size-menu-backdrop';
                backdrop.addEventListener('click', () => toggleSizeMenu(matrixId));
                document.body.appendChild(backdrop);
            }
            
            // Dodaj nagłówek do menu jeśli go nie ma
            if (!menu.querySelector('.size-menu-header')) {
            ...
            // Dla desktop: pozycjonowanie obok ikony
            menu.style.display = 'flex';
...
function updateMatrixVisibility() {
  ...
  const singleMatrixOps = ['det', 'inv', 'trans', 'trace', 'rank', 'eig', 'power'];
...
if (matrixModeSwitch) {
  matrixModeSwitch.addEventListener('change', function() {
    ...
    const wrapper = this.closest('.matrix-mode-switch-wrapper');
    ...
    // Pokaż notyfikację o zmianie trybu
    showModeNotification(isAdvancedMode);
...
function showModeNotification(isAdvanced) {
  const notification = document.createElement('div');
  notification.className = 'mode-notification';
  notification.innerHTML = `
    <i class="fas fa-${isAdvanced ? 'rocket' : 'check-circle'}"></i>
    <span>Tryb ${isAdvanced ? 'rozszerzony' : 'podstawowy'} aktywny</span>
  `;
  notification.style.cssText = `
    position: fixed;
    top: 20px;
    right: 20px;
    background: ${isAdvanced ? 'linear-gradient(135deg, #317873, #399e8c)' : 'linear-gradient(135deg, #e07a5f, #f08a6f)'};
    color: white;
    padding: 15px 25px;
    border-radius: 12px;
    display: flex;
...
function matrixPower(matrix, n) {
    ...
    for (let i = 1; i < n; i++) {
        const temp = Array(matrix.length).fill().map((_, row) =>
            Array(matrix[0].length).fill().map((_, col) =>
                Array(matrix[0].length).fill().reduce((sum, _, k) => 
                    sum + result[row][k] * matrix[k][col], 0
                )
            )
        );
        result = temp;
...
function eigenvalues(matrix) {
    // Uproszczona implementacja dla macierzy 2x2 i 3x3
    const n = matrix.length;
    
    if (n === 2) {
        // Dla macierzy 2x2: rozwiązanie równania charakterystycznego
        const a = matrix[0][0];
        const b = matrix[0][1];
        const c = matrix[1][0];
        const d = matrix[1][1];
        ...
        // Metoda QR dla macierzy 3x3 (uproszczona)
        return "Wartości własne dla macierzy 3x3 wymagają zaawansowanych metod numerycznych";
    } else {
        return "Wartości własne dostępne tylko dla macierzy 2x2";
    }
}
