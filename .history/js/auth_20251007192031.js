...
class AuthSystem {
    ...
    initializeAuthUI() {
        ...
        console.log('AuthSystem ready, dispatching authSystemReady event');
        window.dispatchEvent(new CustomEvent('authSystemReady', { 
            detail: this.currentUser ? { ...this.currentUser, isLoggedIn: true } : null
        }));
    }
    ...
    async handleLogin(e) {
        ...
        if (user) {
            ...
            setTimeout(() => {
                this.closeAuthModal();
                this.updateUserInterface();
                // Dispatch auth state change event with user data
                window.dispatchEvent(new CustomEvent('authStateChanged', { 
                    detail: { ...this.currentUser, isLoggedIn: true }
                }));
            }, 1000);
        } else {
    ...
    performLogout() {
        ...
        // Dispatch auth state change event with null user data
        window.dispatchEvent(new CustomEvent('authStateChanged', { 
            detail: null 
        }));
