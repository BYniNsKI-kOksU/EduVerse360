...
class ModernSidebar {
    ...
    navigateToApp(appName) {
        ...
        if (appId) {
            ...
            if (appElement) {
                ...
                if (dockActionMap[appName]) {
                    this.updateDockActiveState(dockActionMap[appName]);
                }
            }
        }
    ...
    showAbout() {
        ...
        this.updateDockActiveState('about');
    }
    ...
    showSettings() {
        ...
        this.updateDockActiveState('settings');
    }
    ...
    updateMobileDockTooltips() {
        ...
        const tooltips = {
            ...
            'about': this.getTranslation('sidebar.about') || 'O aplikacji',
            'settings': this.getTranslation('sidebar.settings') || 'Ustawienia'
        };
    ...
    // Update dock active state based on current screen
...
class NavbarManager {
    ...
    setupMobileDock() {
        ...
        // Setup carousel scroll indicators
        this.setupDockCarousel(mobileDock);
        
        // Center the home button on load
        setTimeout(() => {
            const homeBtn = mobileDock.querySelector('.dock-home');
            if (homeBtn) {
                homeBtn.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
            }
        }, 600);
    }
    ...
    setupDockCarousel(dock) {
        const updateScrollIndicators = () => {
            ...
            const scrollWidth = dock.scrollWidth;
            const clientWidth = dock.clientWidth;
            
            // Remove existing classes
            dock.classList.remove('can-scroll-left', 'can-scroll-right');
            
            // Add classes based on scroll position
            if (scrollLeft > 10) {
                dock.classList.add('can-scroll-left');
            }
            if (scrollLeft < scrollWidth - clientWidth - 10) {
                dock.classList.add('can-scroll-right');
            }
        };

        // Update on scroll
        dock.addEventListener('scroll', updateScrollIndicators);
        
        // Update on resize
        window.addEventListener('resize', updateScrollIndicators);
        
        // Initial update
        setTimeout(updateScrollIndicators, 700);
        ...
        dock.addEventListener('touchmove', (e) => {
            ...
            dock.scrollLeft = scrollLeftStart + walk;
        });
    }
